...
-   Familiarize with the format for dynamic reports Rmarkdown/quarto

-   Be able to document data management and analysis in R using Rmarkdown/quarto
:::
...
installed <- sapply(ptqs, function(y) {
...
-   **The result is an HTML file** that includes images, code blocks with R commands, the results of these codes, and text. No additional files are needed, everything is embedded in the HTML file.

-   Reports are **easy to share via email or publish online**.
...
your first '.Rmd' file (or R Markdown). The new file already comes with a template with basic instructions:
...
-   Create a dynamic report that includes an interactive map of Costa Rica using the `leaflet` package

 

-   Reproduce one of the examples available on the dygraphs package site (<https://rstudio.github.io/dygraphs/>) and embed it in your dynamic report

 

-   Install the `kableExtra` package and embed in your report the example code from the documentation of the `kable_styling()` function of that package
...
-   Generate a dynamic report in PDF format

 

-   Create an Rstudio project to organize the contents of the course

 

-   Create a research compendium with sketchy
:::
